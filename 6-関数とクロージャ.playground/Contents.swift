...
inoutGreet(user: &inoutUser)  //インアウト引数を指定する場合は、引数の前に&をつける必要がある。
...
func printStrings(strings: String...){  //引数の型指定の後に"..."をつけることで、引数を可変長引数に指定することができる。
    ...
    for element in strings {  //受け取った引数は配列で扱われるので、for文で各要素にアクセスする
        print("strings: \(element)")
    }
}

printStrings(strings: "1-太郎", "2-山田", "間抜け太郎")  //複数の引数を設定することができる。
...
isEqual(100,150)  //100=150ではないため、falseが戻り値として返される。
...
func or1(lhs:Bool,rhs:Bool) -> Bool{
    ...
    if lhs {
        print("lhs-true")
        return true
    } else {     //ここより下の処理は、lhs => falseの場合だけ行われる。
        print("rhs")
        return rhs
    }
}
...
func rhs1() -> Bool {  //ここの処理は、hs() => trueの場合は、行う必要がない。
    print("rhs関数を実行します")
    return false
}
